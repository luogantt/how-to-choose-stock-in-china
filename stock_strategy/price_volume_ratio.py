...
db1 = client1.stock.price_volume_ratio
...
def potential_index(tl):
    ...
    if str(type(df))!="<class 'NoneType'>":
        ...
        if df.shape[0]>250:
            ...
            dfp=df[(df['p_change']>-9.7)&(df['p_change']<9)]
            
            dfp=dfp[dfp['p_change']!=-5]
            pvr=10**6*dfp['p_change']/dfp['volume']
            
            #pvr=pvr[pvr<100]
            
            pvr1=pvr[pvr<0]
            
            pvr2=pvr1.sort_values()
            
            split=int(len(pvr2)*0.03)
            
            pvr3=pvr2.iloc[:split]
            ...
            p1=10**6*df['p_change'].iloc[-1]/df['volume'].iloc[-1]
